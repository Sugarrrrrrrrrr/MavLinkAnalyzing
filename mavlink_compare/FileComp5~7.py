                ...
                if '_source' in jsons[i]:
                    if 'layers' in jsons[i]['_source']:
                        if 'ip' in jsons[i]['_source']['layers']:
                            ...
                            # type
                            if jsons[i]['_source']['layers']['ip']['ip.src_host'] == '10.1.10.10':
                                data_type = 'to'
                            elif jsons[i]['_source']['layers']['ip']['ip.src_host'] == '192.168.1.10':
                                data_type = 'from'
                            else:
                                data_type = 'none'
...
def main():
    ...
    with open('%d.txt'% n, 'w') as file:
        file.write('''%d UDP to 192.168.1.10 in debug
%d lost in wireshark

%d UDP from 192.168.1.10 in wireshark
%d lost in debug

------------------------------
''' % (n_to, len(to_lost_list), n_from, len(from_lost_list)))

        file.write('to 192.168.1.10 lost list:')
        file.write('\n\n')
        ...
        file.write('from 192.168.1.10 lost list:')
        file.write('\n\n')
